...
if [ ! -d src.cln.${SUB} ]; then mkdir src.cln.${SUB}; fi
rm src.cln.${SUB}/*

i=0
for page in images.raw-${SUB}/*.ppm images.raw-${SUB}/*.pbm; do
	if [ ! -f "$page" ]; then continue; fi
	i=$(($i+1))
	p="$(printf "%03d" $i)"
	echo "Processing ${page}"
	dest="src.cln.${SUB}/$p".png
	...
	cyan=src.cln.${SUB}/"$p"-c.tiff
	magenta=src.cln.${SUB}/"$p"-m.tiff
	yellow=src.cln.${SUB}/"$p"-y.tiff
	black=src.cln.${SUB}/"$p"-k.tiff
	tmp=src.cln.${SUB}/"$p"-cmyk.tiff
...
#gm convert src.cln.${SUB}/*.png -filter sinc -compress JPEG -quality 30 -resize "$bksize" src-cln-${SUB}.pdf
